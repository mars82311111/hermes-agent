...
# Default paths
DEFAULT_MEMORY_PATH = Path.home() / ".hermes_memory"
DEFAULT_LANCE_PATH = DEFAULT_MEMORY_PATH / "lance"
DEFAULT_KG_PATH = DEFAULT_MEMORY_PATH / "kg.db"
...
class HermesMemoryProvider:
    ...
    def store(
        ...
        with self._lock:
            ...
            # Use embedder directly (not via retriever which doesn't have embed())
            vector_dim = self._storage.vector_dim if self._storage else 384
            try:
                if self._embedder:
                    ...
                    # Match the table's vector dimension, not hardcoded 384
                    import hashlib
                    h = hashlib.sha256(text.encode()).digest()
                    # Extend hash to required dimension using repeated hashing
                    vector = []
                    seed = text
            ...
            if duplicate_id:
                # Update existing entry: boost importance and refresh timestamp
                try:
    ...
    def get_profile(self) -> Dict[str, Any]:
        ...
        try:
            # Search KG for user facts
            triples = self._kg.search("user preferences")
            
            for triple in triples[:20]:
                if triple.predicate in ["prefers", "likes", "owns"]:
