...
class BackupQueue:
    ...
    def _write_all_unlocked(self, tasks: List[Dict[str, Any]]) -> None:
        # CRITICAL FIX: Ensure parent directory exists before creating tmp file.
        # The directory may be cleaned up by external tools (e.g. tmp cleaners).
        ...
        try:
            ...
            # If atomic replace fails (e.g. cross-device, permission), fall back to direct write.
            logger.warning("Atomic replace failed (%s), falling back to direct write", e)
    ...
    def write_all(self, tasks: List[Dict[str, Any]]) -> None:
        with self._lock:
            self._write_all_unlocked(tasks)
    ...
    def run(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                logger.exception("BackupWorker unexpected error in main loop")
                self._stop_event.wait(self.interval)
        ...
        while popped:
            try:
                ...
                logger.exception("BackupWorker unexpected error during shutdown drain")
                break
            popped, _ = self.queue.atomic_pop_due(self._is_due, self.batch_size)
