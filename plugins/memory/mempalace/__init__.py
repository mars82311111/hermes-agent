...
class _WALBatcher(threading.Thread):
    ...
    _MAX_BATCH = 500         # episodes per batch — was 100, increased to reduce ChromaDB call frequency
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            ...
            if success:
                ...
                # CRITICAL FIX: ChromaDB failed — re-write batch to the FRONT of WAL
                # so entries are NOT lost. Entries were already read from WAL but not
                # cleared, so we prepend them back to preserve ordering.
                _ensure_mempalace_dirs()
                try:
                    _ensure_mempalace_dirs()
                    current_wal_entries = _read_wal()
                    _truncate_wal()
                    wal_path = _wal_path()
                    with open(wal_path, "w", encoding="utf-8") as f:
                        # Write batch at the front (entries that failed to sync)
                        for e in batch:
                            f.write(json.dumps(e, ensure_ascii=False, default=str) + "\n")
                        # Then any new entries that arrived during the failed sync
                        for e in current_wal_entries:
                            f.write(json.dumps(e, ensure_ascii=False, default=str) + "\n")
                    logger.info("WAL recovery: %d entries restored to WAL after ChromaDB failure", len(batch))
                except Exception as e:
                    logger.error("WAL recovery FAILED: %s — entries may be lost", e)
                _update_health(
...
def _has_mempalace_cli() -> bool:
    ...
    return shutil.which(_MEMPALACE_PYTHON) is not None
...
def _cli_search(query: str, wing: str = None, room: str = None, n_results: int = 8) -> str:
    """Run mempalace search via ChromaDB directly (no MemoryStack needed)."""
    script = (
        "import chromadb, json\n"
        "client = chromadb.PersistentClient(path=REPLACEME_PALACE)\n"
        "try:\n"
        ...
        "    where_filter = {}\n"
        "    if REPLACEME_WING:\n"
        "        where_filter['wing'] = REPLACEME_WING\n"
        "    if REPLACEME_ROOM:\n"
        "        where_filter['room'] = REPLACEME_ROOM\n"
        "    if where_filter:\n"
        "        raw = col.query(query_texts=[REPLACEME_QUERY], n_results=REPLACEME_N,\n"
        ...
        "    results = [{'document': d, 'metadata': m, 'distance': dists[i]}\n"
    ...
    ).replace("REPLACEME_ROOM", repr(room)
    ).replace("REPLACEME_N", str(n_results))
...
def _cli_wakeup(wing: str = None) -> str:
    """Run mempalace wake-up via direct file + SQLite access (no MemoryStack needed)."""
    import json as _json
    from pathlib import Path as _Path

    # ── L0: Read identity ────────────────────────────────────────────────────
    identity_path = _Path(_HERMES_IDENTITY)
    identity_content = ""
    ...
    # ── L1: KG active triples ───────────────────────────────────────────────
    kg_path = _Path(_HERMES_KG)
    kg_facts = []
    if kg_path.exists():
        try:
            import sqlite3 as _sqlite3
            conn = _sqlite3.connect(str(kg_path))
            conn.execute("PRAGMA journal_mode=WAL")
            conn.execute("PRAGMA busy_timeout=5000")
            cur = conn.cursor()
            rows = cur.execute(
                "SELECT subject, predicate, object FROM triples "
                "WHERE (valid_to IS NULL OR valid_to='') LIMIT 50"
            ).fetchall()
            for row in rows:
                kg_facts.append({"subject": row[0], "predicate": row[1], "object": row[2]})
            conn.close()
        except Exception:
            pass

    # ── L2: ChromaDB collections summary ──────────────────────────────────────
    palace_path = _Path(_HERMES_PALACE)
    collections_summary = {}
    if palace_path.exists():
        try:
            import chromadb as _chromadb
            client = _chromadb.PersistentClient(path=str(palace_path))
            for col_name in ['mempalace_drawers', 'episodes', 'hermes', 'facts',
                              'preferences', 'convos', 'hermes_memory', 'hermes_user']:
                try:
                    col = client.get_collection(col_name)
                    collections_summary[col_name] = col.count()
                except Exception:
                    collections_summary[col_name] = 0
        except Exception:
            pass

    result = {
        "identity": identity_content[:500] if identity_content else "",
        "kg_facts": kg_facts,
        "collections": collections_summary,
        "palace_path": str(_HERMES_PALACE),
    }
    return _json.dumps(result)


def _cli_status() -> Dict[str, Any]:
    """Run mempalace status via direct ChromaDB + SQLite (no MemoryStack needed)."""
    result = {"error": None, "collections": {}, "kg_entities": 0, "kg_triples": 0}

    # ── ChromaDB collections ───────────────────────────────────────────────
    palace_path = Path(_HERMES_PALACE)
    if palace_path.exists():
        try:
            import chromadb
            client = chromadb.PersistentClient(path=str(palace_path))
            all_cols = ['mempalace_drawers', 'episodes', 'hermes', 'facts',
                        'preferences', 'convos', 'hermes_memory', 'hermes_user']
            for col_name in all_cols:
                try:
                    col = client.get_collection(col_name)
                    result["collections"][col_name] = col.count()
                except Exception:
                    result["collections"][col_name] = 0
        except Exception as e:
            result["error"] = str(e)
    else:
        result["error"] = f"Palace not found: {_HERMES_PALACE}"

    # ── KG stats ──────────────────────────────────────────────────────────
    kg_path = Path(_HERMES_KG)
    if kg_path.exists():
        try:
            import sqlite3
            ...
            conn.execute("PRAGMA journal_mode=WAL")
            cur = conn.cursor()
            result["kg_entities"] = cur.execute(
                "SELECT COUNT(DISTINCT subject) FROM triples WHERE valid_to IS NULL OR valid_to=''"
            ).fetchone()[0] or 0
            result["kg_triples"] = cur.execute(
...
def _openclaw_search(query: str, wing: str = None, room: str = None, n_results: int = 8) -> str:
    """Search OpenClaw palace memories via ChromaDB directly (READ-ONLY, no MemoryStack)."""
    script = (
        "import chromadb, json\n"
        "client = chromadb.PersistentClient(path=REPLACEME_PALACE)\n"
        "try:\n"
...
def _openclaw_wakeup(wing: str = None) -> str:
    """Wake-up OpenClaw palace via direct file + SQLite access (READ-ONLY, no MemoryStack)."""
    import json as _json
    from pathlib import Path as _Path

    # ── L0: Read identity ────────────────────────────────────────────────────
    identity_path = _Path(_OPENCLAW_IDENTITY)
    identity_content = ""
    if identity_path.exists():
        try:
            identity_content = identity_path.read_text(encoding="utf-8").strip()
    ...
    # ── L2: ChromaDB collections summary ──────────────────────────────────────
    palace_path = _Path(_OPENCLAW_PALACE)
    collections_summary = {}
    if palace_path.exists():
        try:
            import chromadb as _chromadb
    ...
    result = {
        ...
        "kg_facts": kg_facts,
        "collections": collections_summary,
        "palace_path": str(_OPENCLAW_PALACE),
    }
    return _json.dumps(result)


def _openclaw_status() -> Dict[str, Any]:
    """Get OpenClaw palace status via direct ChromaDB + SQLite (READ-ONLY, no MemoryStack)."""
    result = {"error": None, "collections": {}, "kg_entities": 0, "kg_triples": 0}

    # ── ChromaDB collections ───────────────────────────────────────────────
    palace_path = Path(_OPENCLAW_PALACE)
    if palace_path.exists():
        try:
            import chromadb
            client = chromadb.PersistentClient(path=str(palace_path))
        ...
        result["error"] = f"OpenClaw palace not found: {_OPENCLAW_PALACE}"

    # ── KG stats ──────────────────────────────────────────────────────────
    kg_path = Path(_OPENCLAW_KG)
    if kg_path.exists():
        try:
            import sqlite3
            conn = sqlite3.connect(str(kg_path))
...
class MemPalaceMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        logger.warning("[DEBUG] MemPalace initialize() ENTRY @ %s", time.strftime("%H:%M:%S"))
        ...
        logger.warning("[DEBUG] MemPalace init: step1 _config START @ %s", time.strftime("%H:%M:%S"))
        logger.warning("[DEBUG] MemPalace init: step1 _config done @ %s", time.strftime("%H:%M:%S"))
        # L3: Restore recent episodic turns from ChromaDB into working memory
        # CRITICAL FIX: Run asynchronously so init doesn't block on slow ChromaDB.
        # Working memory will be populated shortly after agent starts.
        def _async_restore():
            self._restore_episodes(session_id)
            logger.warning("[DEBUG] MemPalace init: step2 _restore_episodes done @ %s", time.strftime("%H:%M:%S"))
        _restore_thread = threading.Thread(target=_async_restore, daemon=True, name="mempalace-restore")
        _restore_thread.start()
        logger.warning("[DEBUG] MemPalace init: step2 _restore_episodes STARTED (async) @ %s", time.strftime("%H:%M:%S"))

        # WAL: Replay any un-flushed episodes to ChromaDB (crash recovery)
        # CRITICAL FIX: Run asynchronously so init doesn't block for 60s on slow ChromaDB.
        # The agent must return from __init__ quickly to remain responsive.
        def _async_wal_replay():
            self._replay_pending_wal()
        _wal_thread = threading.Thread(target=_async_wal_replay, daemon=True)
        _wal_thread.start()
        logger.warning("[DEBUG] MemPalace init: step3 _replay_pending_wal STARTED (async) @ %s", time.strftime("%H:%M:%S"))

        # CRITICAL FIX: steps 3-6 all run as async background tasks.
        # Previous WAL replay thread may still be holding _WAL_LOCK,
        # so we must NOT wait on those threads here.
        def _async_init_remaining():
            # WAL batcher (singleton, idempotent)
            _start_wal_batcher()
            logger.warning("[DEBUG] MemPalace init: step4 _start_wal_batcher done @ %s", time.strftime("%H:%M:%S"))
            # Cloud backup worker (singleton, idempotent)
            _backup.start_backup_worker()
            logger.warning("[DEBUG] MemPalace init: step5 _backup.start_backup_worker done @ %s", time.strftime("%H:%M:%S"))
            # Quick CLI check (standalone mempalace is optional)
            cli_code, _, cli_stderr = _run_cli(["status"])
            logger.warning("[DEBUG] MemPalace init: step6 _run_cli status done @ %s", time.strftime("%H:%M:%S"))
            if cli_code != 0:
                logger.debug("MemPalace standalone CLI unavailable, skipping status check: %s", cli_stderr)
            else:
                logger.info("MemPalace provider initialized. palace_path=%s session=%s",
                            self._palace_path, session_id)
        _t = threading.Thread(target=_async_init_remaining, daemon=True)
        _t.start()

        logger.warning("[DEBUG] MemPalace initialize() returning @ %s", time.strftime("%H:%M:%S"))
    ...
    def on_memory_write(self, action: str, target: str, content: str) -> None:
        """Mirror built-in memory writes to MemPalace L2 drawers.

        Maps:
          - target='memory' -> wing='hermes_memory', hall='facts'
          - target='user'   -> wing='hermes_user',   hall='preferences'
        """
        ...
        wing = "hermes_user" if target == "user" else "hermes_memory"
        hall = "preferences" if target == "user" else "facts"
        importance = 0.8 if target == "user" else 0.7

        try:
            # _handle_add_drawer returns a JSON string; we just fire-and-forget.
            self._handle_add_drawer({
            ...
            logger.debug("MemPalace on_memory_write failed", exc_info=True)
